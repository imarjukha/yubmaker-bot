...
ЗАДАЧА — любое сообщение где есть поручение, действие которое нужно выполнить. Признаки задачи:
- Есть глагол действия: написать, сделать, подготовить, нарисовать, согласовать, выполнить, прислать, выгрузить, починить, установить, договориться и т.п.
- Есть упомянутый исполнитель (@username)
- Может быть указан срок (до пятницы, срок: 30/6, к 30 июня и т.п.)
- Сообщение может быть длинным, со скобками, тире, пояснениями — это не мешает быть задачей
- Вежливая форма ("пожалуйста", "оперативно") не делает сообщение не-задачей

НЕ задача — только: чистые вопросы без поручений, приветствия, обсуждения без конкретного действия.

Если есть хоть малейшее сомнение — считай задачей. (определить, прописать, сделать, подготовить, написать, купить, договориться, прислать, организовать и т.п.) — это ЗАДАЧА, даже если сообщение длинное или содержит контекст и пояснения. Длина сообщения не влияет на то, является ли оно задачей.
