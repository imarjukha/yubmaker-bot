...
ВАЖНО: если в сообщении есть хотя бы один глагол-поручение (определить, прописать, сделать, подготовить, написать, купить, договориться, прислать, организовать и т.п.) — это ЗАДАЧА, даже если сообщение длинное или содержит контекст и пояснения. Длина сообщения не влияет на то, является ли оно задачей.
